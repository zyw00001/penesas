...
class SuperTable extends React.Component {
    ...
    if (col.filter) {
      ...
      col.onFilter = (value, record) => {
        ...
        if(!col.isDateFilterByStartAndEnd){
          ...
          // if((value.start && value.start > content) || (value.end && value.end < content)){
          //   return false;
          // };
          // return true;
        }
  ...
  getProps = () => {
    ...
    const tableClassName = hasUnreadTable === undefined ? s.root : s.noTransition;
    const {onRowClick, ...extraProps} = this.getPropsByCheckbox();
    return {
      ...
      onRow: this.getRowCallback(onRowClick),
      ...extraProps
    };
  };
